...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const amenityId = searchParams.get('amenity')
  ...
  try {
    ...
    const res = await fetch(`${baseUrl}/target-property/${params.id}/amenity/${amenityId}`, {
      method: 'DELETE',
      mode: 'cors',
      headers: {
    ...
    if (response.error) {
      throw new Error('Não foi possivel remover essa amenity do target agora')
    }
